...
export function LinksContent({ blockId }: LinksContentProps) {
  ...
  const allItems = getLinkItemsByBlock(blockId);
  ...
  if (!item) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-zinc-500">
        ...
        <p className="text-sm text-zinc-600">Select a link from the sidebar or create a new one</p>
      </div>
    );
  }
          ...
          <div className="flex gap-2">
            ...
            {item.url && (
              <a
                ...
                className="px-4 py-2 bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-400 rounded-lg transition-colors flex items-center gap-2"
              >
