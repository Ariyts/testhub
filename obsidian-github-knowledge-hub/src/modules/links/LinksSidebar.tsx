...
export function LinksSidebar({ blockId }: LinksSidebarProps) {
  ...
  const handleCreate = () => {
    const item = addLinkItem(blockId, { title: 'New Link', order: items.length });
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between px-3 py-2 border-b border-zinc-800">
        ...
        <button onClick={handleCreate} className="p-1 rounded hover:bg-zinc-800 text-zinc-500 hover:text-zinc-300" title="New link">
          <Plus size={14} />
        </button>
      </div>
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-8 text-zinc-600 text-xs">
            {searchQuery ? 'No links found' : (
              <>
                No links yet.<br />
                <button onClick={handleCreate} className="text-indigo-400 hover:text-indigo-300 mt-2">Add your first link</button>
              </>
            )}
          </div>
        ) : (
              ...
              <div className="space-y-0.5 px-2">
                {categoryItems.map((item) => (
                  <div
                    ...
                    className={cn('group flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors', activeItemId === item.id ? 'bg-zinc-800' : 'hover:bg-zinc-800/50')}
                  >
                    ...
                    {item.url && (
                      <a href={item.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="opacity-0 group-hover:opacity-100 p-1 hover:bg-zinc-700 rounded text-zinc-500 hover:text-zinc-300">
