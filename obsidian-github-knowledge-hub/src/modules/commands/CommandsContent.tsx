...
import { Terminal, Trash2, Tag, X, Plus, Copy, Check, Hash } from 'lucide-react';
  ...
  if (!item) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-zinc-500">
        ...
        <p className="text-sm text-zinc-600">Select a command from the sidebar</p>
      </div>
    );
  }
    ...
    <div className="flex-1 p-8 max-w-2xl mx-auto">
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          />
          <p className="text-sm text-zinc-600 mt-1">Copied {item.copyCount} times</p>
        </div>
      ...
      <div className="space-y-6">
        <div>
          <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wide mb-2">
            <Hash size={12} className="inline mr-1" />Command
          </label>
          <div className="relative">
            ...
            <button
              onClick={handleCopy}
              className={cn('absolute top-3 right-3 p-2 rounded-lg transition-colors', copied ? 'bg-emerald-500/20 text-emerald-400' : 'bg-zinc-800 text-zinc-400 hover:text-zinc-200')}
            >
